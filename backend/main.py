...
def validate_config(config):
    """Fail fast on missing/invalid config entries."""
    required = {
        'data': ['url', 'sequence_length', 'forecast_horizon',
                 'train_ratio', 'val_ratio', 'test_ratio'],
        'training': ['batch_size', 'learning_rate', 'epochs', 'patience',
                     'weight_decay', 'scheduler'],
        'paths': ['model_dir', 'log_dir', 'plot_dir'],
        'ensemble': ['models'],
    }
    ...
    if abs(total - 1.0) > 1e-6:
        raise ValueError(f"data.train/val/test_ratio must sum to 1.0 (got {total})")
    if any(r <= 0 for r in ratios):
        raise ValueError(f"data ratios must be > 0 (got {ratios})")

    if config['training']['batch_size'] <= 0:
        raise ValueError("training.batch_size must be > 0")
    if config['data']['sequence_length'] <= 0 or config['data']['forecast_horizon'] <= 0:
        raise ValueError("sequence_length and forecast_horizon must be > 0")
...
def evaluate_model(model, test_loader, data_loader, model_name, config):
    ...
    evaluator = ModelEvaluator(model, test_loader, device, data_loader, split='test')
    ...
    if args.mode in ['train', 'all']:
        ...
        if args.model == 'all' or args.model == 'ensemble':
            ...
            ensemble_trainer = EnsembleTrainer(
                individual_models, train_loader, val_loader, config, device,
            ...
            for model_type in model_types:
                ...
                if not os.path.exists(model_path):
                    raise FileNotFoundError(
                        ...
                        f"Train it first with: python main.py --mode train --model {model_type}"
                    )
                if model_type == 'ensemble':
                    ensemble_model = create_ensemble(config)
                    checkpoint = torch.load(model_path, map_location=device, weights_only=False)
                    ensemble_model.load_state_dict(checkpoint['model_state_dict'])
                    trained_models[model_type] = ensemble_model.to(device)
                else:
                    model = create_model(model_type, config)
                    checkpoint = torch.load(model_path, map_location=device, weights_only=False)
