...
class Trainer:
    ...
    def train(self, progress_callback=None, cancel_flag=None):
        """Full training loop.

        Args:
            progress_callback: Optional callable invoked after each epoch with
                (epoch_idx, total_epochs, train_loss, val_loss, val_metrics, best_val_loss).
                Epoch index is 1-based.
            cancel_flag: Optional object with an ``is_set() -> bool`` method
                (e.g. ``threading.Event``). Checked after each epoch; when true,
                training stops cooperatively and ``stopped_reason='cancelled'``.
        """
        ...
        for epoch in range(epochs):
            ...
            if improved:
                ...
                # Deep copy — state_dict().copy() shares tensor refs that optimizer.step() mutates in place.
            ...
            if progress_callback is not None:
                try:
                    progress_callback(
                        epoch + 1, epochs, float(train_loss), float(val_loss),
                        val_metrics, float(self.best_val_loss),
                    )
                except Exception as e:
                    print(f"progress_callback error: {e}")

            if not improved and self.patience_counter >= patience:
                print("Early stopping triggered!")
                stopped_reason = 'early_stopped'
                break

            if cancel_flag is not None and cancel_flag.is_set():
                print("Cancellation requested — stopping.")
                stopped_reason = 'cancelled'
                break

        training_time = time.time() - start_time
        print(f"\nTraining completed in {training_time:.2f} seconds ({stopped_reason})")
...
class EnsembleTrainer:
    ...
    def __init__(self, individual_models, train_loader, val_loader, config, device='cpu',
        ...
        if already_trained:
            print("Skipping individual training — using pre-trained models.")
            self.trained_models = [m.to(device) for m in individual_models]
        else:
            self.trained_models = []
