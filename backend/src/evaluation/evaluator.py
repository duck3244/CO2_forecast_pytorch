...
class ModelEvaluator:
    def __init__(self, model, data_loader, device='cpu', scaler=None, split='test'):
        self.model = model
    ...
    def evaluate(self):
        ...
        if self.scaler:
            predictions_orig = self.scaler.inverse_transform_co2(predictions, split=self.split)
            targets_orig = self.scaler.inverse_transform_co2(targets, split=self.split)
        else:
